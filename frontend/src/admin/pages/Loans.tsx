...
import { managedLoans, formatCurrency } from '../mock/adminData';
...
const AdminLoans: React.FC = () => {
  const [loans, setLoans] = useState<ManagedLoan[]>(managedLoans);
  ...
  const handleSave = () => {
    ...
    setLoans((prev) => prev.map((l) =>
      l.id === editLoan.id ? { ...l, status: statusInput, outstanding: parseFloat(outstandingInput) || l.outstanding } : l
    ));
    setEditLoan(null);
  };
      ...
      {editLoan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="text-sm text-slate-500 mb-5">{editLoan.userName} — {formatCurrency(editLoan.amount)}</p>
